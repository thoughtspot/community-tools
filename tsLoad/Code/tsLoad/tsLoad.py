...
class IncomingInterface:
    ...
    def ii_init(self, record_info_in: object) -> bool:
        ...
        if self.parent.alteryx_engine.get_init_var(self.parent.n_tool_id, 'UpdateOnly') == 'False':
            ...
            cmd = 'gzip -dc | tsload --target_database ' + self.parent.targetDatabase + ' --target_table ' + self.parent.tableName + ' --field_separator \',\' --null_value \'\' --date_time_format \'%Y-%m-%d %H:%M:%S\' --has_header_row --date_format \'%Y-%m-%d\' ' + empty_target + ' --boolean_representation \'' + self.parent.booleanString + '\' --max_ignored_rows ' + self.parent.maxIgnoredRows + ' --v ' + str(self.parent.verbosity)
